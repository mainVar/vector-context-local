...
export class Context {
    ...
    constructor(config: ContextConfig = {}) {
        ...
        logger.debug("Context", `🔧 Initialized with ${this.supportedExtensions.length} supported extensions and ${this.ignorePatterns.length} ignore patterns`);
        if (envCustomExtensions.length > 0) {
            logger.debug("Context", `📎 Loaded ${envCustomExtensions.length} custom extensions from environment: ${envCustomExtensions.join(', ')}`);
        }
        if (envCustomIgnorePatterns.length > 0) {
            logger.debug("Context", `🚫 Loaded ${envCustomIgnorePatterns.length} custom ignore patterns from environment: ${envCustomIgnorePatterns.join(', ')}`);
        }
    }
    ...
    async indexCodebase(
        ...
        logger.info("Context", `🚀 Starting to index codebase with ${searchType}: ${codebasePath}`);

        await this.loadIgnorePatterns(codebasePath);

        progressCallback?.({ phase: 'Preparing collection...', current: 0, total: 100, percentage: 0 });
        logger.debug("Context", `Debug2: Preparing vector collection for codebase${forceReindex ? ' (FORCE REINDEX)' : ''}`);
        await this.prepareCollection(codebasePath, forceReindex);
        const collectionName = this.getCollectionName(codebasePath);
        logger.debug("Context", `🎯 Using collection name: ${collectionName}`);

        progressCallback?.({ phase: 'Scanning files...', current: 5, total: 100, percentage: 5 });
        const codeFiles = await this.getCodeFiles(codebasePath);
        logger.info("Context", `📁 Found ${codeFiles.length} code files`);
        ...
        const result = await this.processFileList(
            ...
            (filePath, fileIndex, totalFiles) => {
                ...
                logger.debug("Context", `📊 Processed ${fileIndex}/${totalFiles} files`);
                progressCallback?.({
        ...
        logger.info("Context", `✅ Codebase indexing completed! Processed ${result.processedFiles} files in total, generated ${result.totalChunks} code chunks`);
    ...
    async reindexByChange(
        ...
        if (totalChanges === 0) {
            ...
            logger.debug("Context", "✅ No file changes detected.");
            return { added: 0, removed: 0, modified: 0 };
        }

        logger.info("Context", `🔄 Found changes: ${added.length} added, ${removed.length} removed, ${modified.length} modified.`);
        ...
        logger.info("Context", `✅ Re-indexing complete. Added: ${added.length}, Removed: ${removed.length}, Modified: ${modified.length}`);
    ...
    private async deleteFileChunks(collectionName: string, relativePath: string): Promise<void> {
        ...
        if (results.length > 0) {
            ...
            if (ids.length > 0) {
                ...
                logger.debug("Context", `Deleted ${ids.length} chunks for file ${relativePath}`);
            }
        }
    }
    ...
    async semanticSearch(codebasePath: string, query: string, topK: number = 5, threshold: number = 0.5, filterExpr?: string): Promise<SemanticSearchResult[]> {
        const isHybrid = this.getIsHybrid();
        const searchType = isHybrid === true ? 'hybrid search' : 'semantic search';
        logger.debug("Context", `🔍 Executing ${searchType}: "${query}" in ${codebasePath}`);

        const collectionName = this.getCollectionName(codebasePath);
        logger.debug("Context", `🔍 Using collection: ${collectionName}`);

        const hasCollection = await this.vectorDatabase.hasCollection(collectionName);
        if (!hasCollection) {
            logger.warn("Context", `⚠️  Collection '${collectionName}' does not exist. Please index the codebase first.`);
            return [];
        }
        ...
        if (isHybrid === true) {
            ...
            logger.debug("Context", `🔍 Generating embeddings for query: "${query}"`);
            const queryEmbedding: EmbeddingVector = await this.embedding.embed(query);
            logger.debug("Context", `✅ Generated embedding vector with dimension: ${queryEmbedding.vector.length}`);
            logger.debug("Context", `🔍 First 5 embedding values: [${queryEmbedding.vector.slice(0, 5).join(', ')}]`);
            ...
            logger.debug("Context", `🔍 Search request 1 (dense): anns_field="${searchRequests[0].anns_field}", vector_dim=${queryEmbedding.vector.length}, limit=${searchRequests[0].limit}`);
            logger.debug("Context", `🔍 Search request 2 (sparse): anns_field="${searchRequests[1].anns_field}", query_text="${query}", limit=${searchRequests[1].limit}`);

            logger.debug("Context", "🔍 Executing hybrid search with RRF reranking...");
            ...
            logger.debug("Context", `🔍 Raw search results count: ${searchResults.length}`);
            ...
            logger.debug("Context", `✅ Found ${results.length} relevant hybrid results`);
            if (results.length > 0) {
                logger.debug("Context", `🔍 Top result score: ${results[0].score}, path: ${results[0].relativePath}`);
            }
            ...
            logger.debug("Context", `✅ Found ${results.length} relevant results`);
            return results;
        }
    }
    ...
    async clearIndex(
        ...
        logger.info("Context", `🧹 Cleaning index data for ${codebasePath}...`);
        ...
        logger.info("Context", "✅ Index data cleaned");
    }
    ...
    updateIgnorePatterns(ignorePatterns: string[]): void {
        ...
        logger.debug("Context", `🚫 Updated ignore patterns: ${ignorePatterns.length} new + ${DEFAULT_IGNORE_PATTERNS.length} default = ${this.ignorePatterns.length} total patterns`);
    }
    ...
    addCustomIgnorePatterns(customPatterns: string[]): void {
        ...
        patternSet.forEach(pattern => uniquePatterns.push(pattern));
        this.ignorePatterns = uniquePatterns;
        logger.debug("Context", `🚫 Added ${customPatterns.length} custom ignore patterns. Total: ${this.ignorePatterns.length} patterns`);
    }

    resetIgnorePatternsToDefaults(): void {
        this.ignorePatterns = [...DEFAULT_IGNORE_PATTERNS];
        logger.debug("Context", `🔄 Reset ignore patterns to defaults: ${this.ignorePatterns.length} patterns`);
    }

    updateEmbedding(embedding: Embedding): void {
        this.embedding = embedding;
        logger.debug("Context", `🔄 Updated embedding provider: ${embedding.getProvider()}`);
    }
    ...
    private async prepareCollection(codebasePath: string, forceReindex: boolean = false): Promise<void> {
        ...
        logger.debug("Context", `🔧 Preparing ${collectionType} collection for codebase: ${codebasePath}${forceReindex ? ' (FORCE REINDEX)' : ''}`);
        const collectionName = this.getCollectionName(codebasePath);

        const collectionExists = await this.vectorDatabase.hasCollection(collectionName);

        if (collectionExists && !forceReindex) {
            logger.debug("Context", `📋 Collection ${collectionName} already exists, skipping creation`);
            return;
        }

        if (collectionExists && forceReindex) {
            logger.debug("Context", `🗑️  Dropping existing collection ${collectionName} for force reindex...`);
            await this.vectorDatabase.dropCollection(collectionName);
            logger.debug("Context", `✅ Collection ${collectionName} dropped successfully`);
        }

        logger.debug("Context", `🔍 Detecting embedding dimension for ${this.embedding.getProvider()} provider...`);
        const dimension = await this.embedding.detectDimension();
        logger.debug("Context", `📏 Detected dimension: ${dimension} for ${this.embedding.getProvider()}`);
        ...
        logger.debug("Context", `✅ Collection ${collectionName} created successfully (dimension: ${dimension})`);
    }
    ...
    private async processFileList(
        ...
        for (let i = 0; i < filePaths.length; i++) {
            ...
            try {
                ...
                for (const chunk of chunks) {
                    ...
                    if (chunkBuffer.length >= EMBEDDING_BATCH_SIZE) {
                        try {
                            ...
                            logger.error("Context", `❌ Failed to process chunk batch for ${searchType}:`, error);
                    ...
                    if (totalChunks >= CHUNK_LIMIT) {
                        logger.warn("Context", `⚠️  Chunk limit of ${CHUNK_LIMIT} reached. Stopping indexing.`);
                        limitReached = true;
                        break;
                    }
                }
                ...
                logger.warn("Context", `⚠️  Skipping file ${filePath}: ${error}`);
            }
        }

        if (chunkBuffer.length > 0) {
            const searchType = isHybrid === true ? 'hybrid' : 'regular';
            logger.debug("Context", `📝 Processing final batch of ${chunkBuffer.length} chunks for ${searchType}`);
            try {
                await this.processChunkBuffer(chunkBuffer);
            } catch (error) {
                logger.error("Context", `❌ Failed to process final chunk batch for ${searchType}:`, error);
                if (error instanceof Error) {
                    logger.error("Context", 'Stack trace:', error.stack);
                }
            }
    ...
    private async processChunkBuffer(chunkBuffer: Array<{ chunk: CodeChunk; codebasePath: string }>): Promise<void> {
        ...
        const isHybrid = this.getIsHybrid();
        const searchType = isHybrid === true ? 'hybrid' : 'regular';
        logger.debug("Context", `🔄 Processing batch of ${chunks.length} chunks (~${estimatedTokens} tokens) for ${searchType}`);
    ...
    private async processChunkBatch(chunks: CodeChunk[], codebasePath: string): Promise<void> {
        ...
        if (isHybrid === true) {
            ...
            logger.debug("Context", `✅ Successfully inserted ${documents.length} documents`);
        }
    }
    ...
    static async getIgnorePatternsFromFile(filePath: string): Promise<string[]> {
        try {
            ...
            logger.warn("Context", `⚠️  Could not read ignore file ${filePath}: ${error}`);
            return [];
        }
    }
    ...
    private async loadIgnorePatterns(codebasePath: string): Promise<void> {
        try {
            ...
            if (fileBasedPatterns.length > 0) {
                ...
                logger.debug("Context", `🚫 Loaded total ${fileBasedPatterns.length} ignore patterns from all ignore files`);
            } else {
                logger.debug("Context", "📄 No ignore files found, keeping existing patterns");
            }
        } catch (error) {
            logger.warn("Context", `⚠️ Failed to load ignore patterns: ${error}`);
        }
    }
    ...
    private async findIgnoreFiles(codebasePath: string): Promise<string[]> {
        try {
            ...
            if (ignoreFiles.length > 0) {
                logger.debug("Context", `📄 Found ignore files: ${ignoreFiles.map(f => path.basename(f)).join(', ')}`);
            }

            return ignoreFiles;
        } catch (error) {
            logger.warn("Context", `⚠️ Failed to scan for ignore files: ${error}`);
            return [];
        }
    }
    ...
    private async loadIgnoreFile(filePath: string, fileName: string): Promise<string[]> {
        try {
            ...
            logger.debug("Context", `📄 Found ${fileName} file at: ${filePath}`);

            const ignorePatterns = await Context.getIgnorePatternsFromFile(filePath);

            if (ignorePatterns.length > 0) {
                logger.debug("Context", `🚫 Loaded ${ignorePatterns.length} ignore patterns from ${fileName}`);
    ...
    private getCustomExtensionsFromEnv(): string[] {
        ...
        try {
            ...
            logger.warn("Context", `⚠️  Failed to parse CUSTOM_EXTENSIONS: ${error}`);
            return [];
        }
    }
    ...
    private getCustomIgnorePatternsFromEnv(): string[] {
        ...
        try {
            ...
            logger.warn("Context", `⚠️  Failed to parse CUSTOM_IGNORE_PATTERNS: ${error}`);
            return [];
        }
    }
    ...
    addCustomExtensions(customExtensions: string[]): void {
        ...
        logger.debug("Context", `📎 Added ${customExtensions.length} custom extensions. Total: ${this.supportedExtensions.length} extensions`);
    }
